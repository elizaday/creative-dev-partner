...
function buildPrompt(frame, conceptTitle, conceptDescription) {
  return [
    ...
    `Visual decision: ${frame?.visualDecision || frame?.visual || ''}`,
    `Reason for frame: ${frame?.whyThisExists || ''}`,
    `Cut logic: ${frame?.cutLogic || frame?.transition || ''}`,
    `Contrast from previous: ${frame?.contrastFromPrevious || ''}`,
    `Action detail: ${frame?.action || ''}`,
    `Audio context: ${frame?.audio || ''}`,
    conceptTitle || '',
    conceptDescription || '',
